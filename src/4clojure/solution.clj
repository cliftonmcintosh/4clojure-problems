...
;; order; the first player is said to "lead" to the trick. After all players
;; have played, one card is said to have "won" the trick. How the winner is
...
;; particular suit will be designated "trump", meaning that its cards are more
;; powerful than any others: if there is a trump suit, and any trumps are
;; played, then the highest trump wins regardless of what was led.
...
;; with its eight neighbours (horizontal, vertical, diagonal), and its next
;; state is dependent on the following rules:
  ...
  (let [row-length (count (first rs))
        ...
        (fn [r c]
          (let [p-row (dec r)
                ...
                n-column (inc c)]
            (count (filter #(= \# %)
                           [(get-in rs [p-row p-column])
                            (get-in rs [p-row c])
                            (get-in rs [p-row n-column])
                            (get-in rs [r p-column])
                            (get-in rs [r n-column])
                            (get-in rs [n-row p-column])
                            (get-in rs [n-row c])
                            (get-in rs [n-row n-column])]))))]
    (map #(apply str %)
         (partition row-length
                    (for [r (range (count rs))
                          c (range row-length)]
                      (let [cell (get-in rs [r c])
                            live-neighbors (neighbors-fn r c)
                            is-live? (= \# cell)]
                        (cond (and is-live? (<= 2 live-neighbors 3))
                              \#
                              (and (not is-live?) (= 3 live-neighbors))
                              \#
                              :else
                              \space)))))))
